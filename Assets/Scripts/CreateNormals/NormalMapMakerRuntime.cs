...
namespace Rendergon.CreateNormals
{
	public class NormalMapMakerRuntime : MonoBehaviour
	{
		...
		public void ConvertToNormalMap(int filterSize, int normalStrength)
		{
			...
			Texture2D thisTex = (Texture2D)GetComponent<Renderer>().material.GetTexture("_BaseMap");

			// OPTIONAL: filter the texture first before creating normal map
			Debug.Log("Filtering texture..");
			Texture2D filteredTex = NormalMapTools.FilterMedian(thisTex, filterSize);//5

			// create normal map
			Debug.Log("Creating normal map");
			Texture2D normalTex = NormalMapTools.CreateNormalmap(filteredTex, normalStrength);//6
			...
			Debug.Log("Ended making normal map");
