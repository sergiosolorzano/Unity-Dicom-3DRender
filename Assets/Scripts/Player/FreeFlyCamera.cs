...
#endif
        public void PositionThirdPersonInFrontOfImageOrPixel(ref List<DicomParameters> CutStudyDicomParamList)
        {
            Performance_Metrics.Watch thisWatch = new Performance_Metrics.Watch("PositionThirdPersonInFrontOfImageOrPixel");
            
            /*double index = 0;
            Vector3 targetSpawn = new Vector3(0, 0, 0);

            if (CutStudyDicomParamList.Count > 0)
            {
                index = (Math.Round((double)(CutStudyDicomParamList.Count) / 2, MidpointRounding.AwayFromZero) - 1);
                targetSpawn = CutStudyDicomParamList[Convert.ToInt32(index)].ImagePlaneGO.transform.position;
                cam.transform.position = targetSpawn;
            }*/
